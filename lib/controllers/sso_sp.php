<?php
  ...
  if (isset($_POST['submit']) && wp_verify_nonce($_POST['_wpnonce'],'sso_sp') )
  {
      if( isset($_POST['auto_cert']) )
      {
        ...
        $upload_dir = constant('SAMLAUTH_CONF') . '/certs/' . $main_blog_id;
        ...
        $cert_uploaded = ( file_put_contents($upload_dir . '/' . $main_blog_id . '.cer', $certfile) ) ? true : false ;
        $key_uploaded = ( file_put_contents($upload_dir . '/' . $main_blog_id . '.key', $keyfile) ) ? true : false ;
      ...
      elseif( ( isset($_FILES['certificate']) && isset($_FILES['privatekey']) ) && ( $_FILES['certificate']['error'] == 0 && $_FILES['privatekey']['error'] == 0 ) )
      {
          ...
          if(openssl_x509_check_private_key($cert,$key))
          {
              ...
              $cert_uploaded = ( file_put_contents($upload_dir . '/' . $main_blog_id . '.cer', $cert) ) ? true : false ;
              $key_uploaded = ( file_put_contents($upload_dir . '/' . $main_blog_id . '.key', $key) ) ? true : false ;
          }
          else
          {
